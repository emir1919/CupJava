...
public class ProductDetailController implements Initializable {
    ...
    private void addReview(ActionEvent event) {
        ReviewService service = new ReviewService();
        Review r = new Review(commentTF.getText(), java.sql.Date.valueOf(LocalDate.now()), (int) ratingB.getRating(), this.getId(), cupcake.Cupcake.user.getId());
        service.addReview(r);
        reviewsLV.setItems(FXCollections.observableArrayList(service.getProductReviews(id)));
    }
    ...
    private void goHome(ActionEvent event) {
        HomeController.afficherproduits = 1;
        navigateTo(getClass().getResource("/edu/cupcake/gui/Home.fxml"));
    }
    ...
    static class ReviewCell extends JFXListCell<Review>{
        ...
        protected void updateItem(Review item, boolean empty) {
            ...
            if(item != null && !empty){
                ...
                if(cupcake.Cupcake.user != null && item.getUser_id() == cupcake.Cupcake.user.getId()){
